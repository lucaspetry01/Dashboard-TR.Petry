...
# Substitua a linha existente do título (linha 68)
# st.markdown("<h1 style='text-align: center; color: #03DAC6;'>Dashboard | TR. Petry</h1>", unsafe_allow_html=True)

# Substitua-a por este novo bloco de código logo após a definição do estilo (após a linha 65)
st.markdown("""
<div style="
    background-color: #1E1E1E;
    padding: 20px;
...
">
    <h1 style="
        ...
        letter-spacing: 2px;
    ">
        Dashboard <span style="color: #BB86FC;">|</span> TR. Petry
    </h1>
    <p style="
        text-align: center;
        color: #E0E0E0;
        ...
        margin-top: 10px;
    ">
        Análise de Faturamento e Desempenho
    </p>
</div>
""", unsafe_allow_html=True)
...
with col_grafico_placa:
    ...
    # Adicionando barras verticais para o Mês Atual (cor alterada)
    fig_faturamento_placa.add_trace(go.Bar(
        x=df_faturamento_placa['Placa(s)'],
        y=df_faturamento_placa['Mês Atual'],
        name='Mês Atual',
        marker_color='#03DAC6'  # Alterado para a cor da barra de load
    ))
...
with col_grafico_rota:
    ...
    fig_faturamento_rota.add_trace(go.Bar(
        x=df_faturamento_rota['Rota'], 
        y=df_faturamento_rota['Mês Atual'], 
        name='Mês Atual',
    ...
    fig_faturamento_rota.add_trace(go.Scatter(
        x=df_faturamento_rota['Rota'], 
        y=df_faturamento_rota['Mês Anterior'], 
        mode='lines+markers',
        name='Mês Anterior', 
        line=dict(color='red', width=3), 
        marker=dict(symbol='diamond', size=8, color='red'),
